...
export const emptyMacroForm = (): MacroForm => {
  ...
  return {
    ...
    sport: "handball", startDate: start, endDate: end, totalHoursPlanned: "300",
...
const PHASE_KEYS = ["pg", "ps", "comp", "trans"] as const;
...
const MacroCycleForm = ({ editingId, initialData, athletes, onSaved, onCancel }: MacroCycleFormProps) => {
  ...
  const [selectedAthletes, setSelectedAthletes] = useState<Set<string>>(new Set(athletes.map((a) => a._id).filter(() => false)));
  ...
  const handleSave = async () => {
    ...
    if (editingId) {
      await updateMacro({ id: editingId, name: form.name, startDate: form.startDate, endDate: form.endDate, phases, athleteIds, totalHoursPlanned: form.totalHoursPlanned ? Number(form.totalHoursPlanned) : undefined });
      macroId = editingId;
      toast({ description: t("macrocycleForm.updated") });
    } else {
      macroId = await createMacro({ name: form.name, sport: form.sport, startDate: form.startDate, endDate: form.endDate, phases, athleteIds, totalHoursPlanned: form.totalHoursPlanned ? Number(form.totalHoursPlanned) : undefined });
      toast({ description: t("macrocycleForm.created") });
    }
    for (const id of athleteIds) await updateAthlete({ id, macroCycleId: macroId });
    onSaved();
  };

  const phaseColors = {
    pg: "bg-blue-500/20 border-blue-500/30",
    ps: "bg-violet-500/20 border-violet-500/30",
    comp: "bg-yellow-500/20 border-yellow-500/30",
    trans: "bg-green-500/20 border-green-500/30",
  };
        ...
        >
          <div className="flex items-center justify-between">
            <h2 className="font-display font-bold text-xl">
              {editingId ? t("macrocycleForm.edit") : t("macrocycleForm.new")}
            </h2>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="col-span-2 space-y-1.5">
              <Label className="text-sm text-muted-foreground">{t("macrocycleForm.name")}</Label>
              <Input value={form.name} onChange={(e) => f("name", e.target.value)} className="bg-secondary/50 border-border/50" />
            </div>
            <div className="space-y-1.5">
              <Label className="text-sm text-muted-foreground">{t("macrocycleForm.seasonStart")}</Label>
              <Input type="date" value={form.startDate} onChange={(e) => recalcPhases(e.target.value, form.endDate)} className="bg-secondary/50 border-border/50" />
            </div>
            <div className="space-y-1.5">
              <Label className="text-sm text-muted-foreground">{t("macrocycleForm.seasonEnd")}</Label>
              <Input type="date" value={form.endDate} onChange={(e) => recalcPhases(form.startDate, e.target.value)} className="bg-secondary/50 border-border/50" />
            </div>
            <div className="space-y-1.5">
              <Label className="text-sm text-muted-foreground">{t("macrocycleForm.hoursPlanned")}</Label>
          ...
          <div className="space-y-3">
            <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">{t("macrocycleForm.phases")}</p>
            {PHASE_KEYS.map((pk) => (
              <div key={pk} className={`rounded-lg border p-3 space-y-2 ${phaseColors[pk]}`}>
                <p className="text-xs font-medium">{t(`macrocycleForm.phaseLabels.${pk}`)}</p>
                <div className="grid grid-cols-2 gap-2">
                  <div className="space-y-1">
                    <Label className="text-xs text-muted-foreground">{t("macrocycleForm.start")}</Label>
                    <Input type="date" value={form[`${pk}_start`]} onChange={(e) => f(`${pk}_start`, e.target.value)} className="bg-background/50 border-border/30 h-8 text-sm" />
                  </div>
                  <div className="space-y-1">
                    <Label className="text-xs text-muted-foreground">{t("macrocycleForm.end")}</Label>
                    <Input type="date" value={form[`${pk}_end`]} onChange={(e) => f(`${pk}_end`, e.target.value)} className="bg-background/50 border-border/30 h-8 text-sm" />
                  </div>
                </div>
              </div>
            ))}
          ...
          {athletes.length > 0 && (
            <div className="space-y-2">
              <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">
                {t("macrocycleForm.athletesSelected", { count: selectedAthletes.size })}
              </p>
              <div className="grid grid-cols-2 gap-1.5 max-h-40 overflow-y-auto">
                {athletes.map((a) => (
                  <label key={a._id} className="flex items-center gap-2 p-2 rounded-lg hover:bg-secondary/50 cursor-pointer text-sm">
                    <input type="checkbox" checked={selectedAthletes.has(a._id)} onChange={() => toggleAthlete(a._id)} className="accent-primary" />
                    {a.name}
                  </label>
                ))}
              </div>
          ...
          <div className="flex gap-3 pt-2">
            <Button variant="outline" onClick={onCancel} className="flex-1">{t("common:actions.cancel")}</Button>
