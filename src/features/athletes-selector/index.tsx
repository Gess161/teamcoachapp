...
const AthletesSelector = ({ trainingId, currentAthleteIds, onClose }: AthletesSelectorProps) => {
  ...
  const addAthlete = async (athleteId: Id<"athletes">) => {
    ...
    toast({ title: t("athletesSelector.added") });
  };

  const removeAthlete = async (athleteId: Id<"athletes">) => {
    await updateTraining({ id: trainingId, athleteIds: currentAthleteIds.filter((id) => id !== athleteId) });
    toast({ title: t("athletesSelector.removed") });
  };
        ...
        >
          <div className="flex items-center justify-between">
            <h2 className="font-display font-bold text-xl flex items-center gap-2">
              <Users className="w-5 h-5 text-blue-400" /> {t("athletesSelector.title")}
            </h2>
            ...
            {athletes.length > 0 ? (
              athletes.map((athlete) => {
                ...
                return (
                  <div
                    key={athlete._id}
                    onClick={() => isSelected ? removeAthlete(athlete._id) : addAthlete(athlete._id)}
                    className={`p-3 rounded-lg transition-all cursor-pointer ${
                      isSelected ? "bg-primary/20 border border-primary" : "bg-secondary/30 border border-border/50 hover:border-border"
                    }`}
                  >
                    <div className="flex items-center justify-between">
                      <div>
              ...
              <p className="text-sm text-muted-foreground text-center py-8">{t("athletesSelector.notFound")}</p>
            )}
          </div>
