...
const CalendarPage = () => {
  ...
  const trainingEvents = useMemo(() => {
    const events: Record<string, { name: string; time: string; athletes: number }[]> = {};
    if (allTrainings) {
      allTrainings.forEach((training) => {
        if (!events[training.date]) events[training.date] = [];
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="glass-card p-6 space-y-4">
            <h3 className="font-display font-semibold">
              {selectedDate
                ? new Date(selectedDate + "T00:00:00").toLocaleDateString(undefined, { day: "numeric", month: "long" })
                : t("selectDate")}
            </h3>
                ...
                {selectedEvents.map((ev, i) => (
                  <div key={i} className="p-4 rounded-lg bg-secondary/30 space-y-2">
                    ...
                    <div className="text-xs text-muted-foreground flex gap-3">
                      <span>🕐 {ev.time}</span>
                      <span>👥 {ev.athletes} {t("athletes")}</span>
                    </div>
                  </div>
                ))}
              </div>
