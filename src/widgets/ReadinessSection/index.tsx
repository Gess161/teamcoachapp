...
interface ScoreEntry {
  _id: string; athleteId: Id<"athletes">; date: string;
  igs?: number; physical?: number; technical?: number; tactical?: number;
  psychological?: number; functional?: number; coordination?: number; recovery?: number;
}
...
const ReadinessSection = ({
  allScores, athletes, onAddScore, selectedAthleteForRadar, onAthleteChange, readinessRadar,
  ...
  if (allScores.length === 0) {
    return (
      <div className="glass-card p-8 text-center border border-dashed border-border/50">
        ...
        <p className="text-muted-foreground">{t("readiness.noScores")}</p>
        <p className="text-sm text-muted-foreground/60 mt-1 mb-4">{t("readiness.noScoresHint")}</p>
        <Button onClick={onAddScore} variant="outline" className="gap-2">
          <Plus className="w-4 h-4" /> {t("readiness.addScore")}
        </Button>
      </div>
    );
  }
  ...
  const getIgsValue = () => {
    if (selectedAthleteForRadar === "sum") {
      ...
      const sum = Array.from(latestByAthlete.values()).reduce((acc, s) => acc + (s.igs ?? 0), 0);
      return Math.round(sum / latestByAthlete.size);
    } else {
      return allScores.find((s) => s.athleteId === selectedAthleteForRadar)?.igs;
    }
  };
    ...
    <div className="glass-card p-6 space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h2 className="font-display font-semibold">{t("readiness.title")}</h2>
        </div>
        <select
        ...
        >
          <option value="sum">{t("readiness.teamAverage")}</option>
      ...
      {readinessRadar ? (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-center">
          <ResponsiveContainer width="100%" height={260}>
            <RadarChart data={readinessRadar}>
              ...
              <Radar name={t("readiness.readiness")} dataKey="value" stroke="hsl(84,81%,44%)" fill="hsl(84,81%,44%)" fillOpacity={0.3} />
              <Tooltip
              ...
              <div className="mt-4 p-4 rounded-lg bg-primary/10 space-y-3">
                <div>
                  <p className="text-xs text-muted-foreground">
                    ІГС{" "}
                    {selectedAthleteForRadar === "sum" ? t("readiness.igsTeam") : t("readiness.igsIndividual")}
                  </p>
                ...
                {selectedAthleteForRadar !== "sum" && <IGSColorBar variant="full" />}
                <p className="text-xs text-muted-foreground">
                  {selectedAthleteForRadar === "sum"
                    ? t("readiness.teamLevel", { count: athletes.length })
                    : igsValue >= 85
                    ? t("readiness.excellent")
                    : igsValue >= 70
                    ? t("readiness.good")
                    : igsValue >= 55
                    ? t("readiness.satisfactory")
                    : t("readiness.needsWork")}
                </p>
              </div>
            ) : null}
          </div>
        ...
        <p className="text-sm text-muted-foreground text-center py-4">{t("readiness.noData")}</p>
      )}
    </div>
  );
};
